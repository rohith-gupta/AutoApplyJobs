...
- `raw_job_posting` is immutable and owned solely by ingestion. `raw_job_processing` is the separate, mutable Job Processing record — one row per `(raw_job_posting_id, normalizer_version)`, updated in place on retry (`processing_status`, `attempt_count`, `error_message`, `processed_at`); no per-attempt history is modeled. `normalizer_version` is required *when the row is created* (`NOT NULL` as of `V3`), not merely once processing reaches `PROCESSED` — a row is only ever created after the application has selected which normalizer will process that raw posting; a different normalizer version means a different row.
