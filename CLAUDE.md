...
- Two backend services split along data ownership:
  ...
  - **`job-platform-service`** owns everything else: Job Processing (`company`, `job`, `job_source`, `job_location`, `skill`, `skill_alias`, `job_skill`, `raw_job_processing`), Matching (`job_match`, `job_match_skill`), User/Resume, Applications — and exposes the REST API to the frontend
